# A teacher is conducting a camp for a group of five children. Based on their performance and behavior during the
# camp, the teacher rewards them with chocolates.
# Write a Python function to
# 1. Find the total number of chocolates received by all the children put together. Assume that each child is
# identified by an id and it is stored in a tuple and the number of chocolates given to each child is stored in a list.
...
def reward_child(child_id_rewarded, extra_chocolates) :
    if extra_chocolates < 1 :
        ...
        if child_id_rewarded not in child_id :
            ...
            d = chocolates_received[child_id.index(child_id_rewarded)] + extra_chocolates
            chocolates_received[child_id.index(child_id_rewarded)] = d
